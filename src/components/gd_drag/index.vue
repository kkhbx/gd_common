<template>
  <div class="my-drag gd-drag" :class="classes">
    <slot></slot>
  </div>
</template>
  ...
  methods: {
    ...
    getRange() {
      ...
      if (target) {
        ...
        this.cacheRange = typeof this.range === 'function' ? this.range() : this.range || DEFAULT_RANGE
      }
     ...
     */
    userSelect(disabled) {
      disabled ? addClass(this.document.body, 'user-select-none') : removeClass(this.document.body, 'user-select-none')
    },
    // 拖拽开始
    start(e) {
      this.cacheRange = null
