...
const layouts = {
  ...
  gdTable(h, currentItem, index, list) {
    console.log(currentItem.__config__.tableColumn, 'currentItem')
    const { activeItem } = this.$listeners
    const config = currentItem.__config__
    const className = this.activeId === config.formId ? 'drawing-row-item active-from-item' : 'drawing-row-item'
    let child = renderChildren.apply(this, arguments)
    return (
      <el-row>
        <gd-table-test ref="gdtable" columns={currentItem.__config__.tableColumn} data={currentItem.__config__.tableData}></gd-table-test>
        {components.itemBtns.apply(this, arguments)}
      </el-row>
    )
  },
